...
// // [ 계획 관련 함수 ]
// int MakePlan(int userIdix, char planName[], char explain[], int openLevel, char endAt[]); // 계획 생성
// int GetPlanLen(int useirIdx); // 유저가 생성한 계획의 수 리턴
// int GetPlan(int userIdx); // 유저가 생성한 계획 리스트 출력
// int GetFriendPlan(inti userIdx, int friendIdx); // 친구가 생성한 계획 리스트 출력
// void GetDayPlan(int i* arr, int userIdx, char date[]); // 특정 날의 계획 리스트 출력 및 인덱스 리턴
// void DeletePlan(inti userIdx, int planIdx); // 계획 삭제
...
// int GetPlanDetail(iint planIdx); // 유저가 생성한 계획 디테일 리스트 출력
